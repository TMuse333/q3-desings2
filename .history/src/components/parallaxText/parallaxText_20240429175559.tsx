...
const StickyImage = ({ imgUrl }: { imgUrl: string, textPosition: number }) => {
  ...
  const [brightnessValue, setBrightnessValue] = useState(0.2);
  ...
  const brightness = useTransform(
    scrollYProgress,
    [0, 1],  // Range of scrollYProgress values
    [0.2, 1] // Range of brightness values
  );

  useEffect(() => {
    const unsubscribe = brightness.on("change", (value) => {
...
const OverlayCopy = ({
  ...
  export const [textPosition, setTextPosition] = useState(0)
    // Log the y position
    useEffect(() => {
