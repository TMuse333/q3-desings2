...
interface SubMenuProps {
  ...
  = ({subMenuClicked, setSubMenuClicked}) => {
    ...
    return (
        ...
        h-screen w-[150px] z-41 transition-translate hover:bg-green-200
        ${!subMenuClicked ? 'translate-x-[100%]' : ''}`}>
        <div className="relative h-full w-full z-45">
            <p onClick={handleSubMenuLeave}
            className="absolute top-[5%] hover:bg-red-200 left-5 z-45">X</p>
        </div>
        </nav>
    )
  }
...
const Navbar: React.FC<NavbarProps> = ({ links }) => {
    ...
    return (
        <nav className="fixed top-0 z-45 w-screen left-0 bg-q-blue h-[100px] flex justify-between items-center
        ">
            ...
            />
             <div className="relative w-full h-full flex items-center justify-end z-1
             ">
                {!subMenuClicked && (
