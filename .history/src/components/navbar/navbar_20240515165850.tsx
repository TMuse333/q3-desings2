    ...
    return (
      ...
      ${!subMenuClicked ? 'translate-x-full' : 'translate-x-0'}`}>
        <div className="relative h-full w-full z-70">
            ...
            className="absolute top-[5%] right-[10%] z-[200]">X
            </p>
            <ul
                        className={`text-left mt-5 fixed right-[5%] top-[70px] overflow-hidden transition-[height] flex flex-col items-start justify-center rounded-lg w-[140px] lg:bg-transparent lg:w-auto lg:relative lg:overflow-auto lg:top-auto lg:right-auto lg:flex-row `}
                    >
                        {links.map((link, index) => (
                            <Link key={index} to={link.destination}>
                                <li className="text-xs lg:text-lg mb-3
                                mr-auto
