...
hasDescription}) =>{
    ...
    function handlePrevClick() {
      // if (shift === 0) {
      //   setShift(-(images.length * 3 - 1));
      // } else {
      //   setShift(prev => prev + 1);
      // }
      setCurrentImage(prev => (prev === 0 ? infiniteImages.length - 1 : prev - 1));
    }
    
    function handleNextClick() {
      if (shift === -(images.length * 3 - 1)) {
        setShift(0);
      } else {
        setShift(prev => prev - 1);
      }
      setCurrentImage(prev => (prev === infiniteImages.length - 1 ? 0 : prev + 1));
    }
...
'>
  ...
  "
      ...
      >
        ...
        md2:pl-5'>{infiniteImages[currentImage].title}</h1>
        <p className="text-white
        text-center pl-5 pr-5 pt-5
        md2:pr-0 pl-0 md2:text-left
       ">{infiniteImages[currentImage].description}</p>
      </motion.div>
    </AnimatePresence>
       )}
       </section>
