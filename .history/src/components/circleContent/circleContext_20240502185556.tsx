...
const CircleContent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            c.arc(canvasSize.width * 2/3, canvasSize.height / 2, 75, 0,  ( 3*Math.PI)/3);
            c.lineWidth = 4; // Set border width
            c.strokeStyle = 'red'
            c.stroke();
    ...
    const increaseFraction = () => {
        if (fraction < 3) { // Adjust the condition as needed
            setFraction(prevFraction => prevFraction + 0.01); // Adjust the increment as needed
        }
    };
    
    // Call increaseFraction to gradually increase the fraction
    useEffect(() => {
        const intervalId = setInterval(increaseFraction, 100); // Adjust the interval time as needed
        return () => clearInterval(intervalId); // Cleanup function to clear the interval
    }, []);
