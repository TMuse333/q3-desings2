...
const AppearingCircle: React.FC<CircleProps> = ({image,secondCircleComplete,
    ...
    = useState<number>(window.inner)

    const [circleThickness] = useState<number>(4)

    const {isMobile2} = useGeneralContext()

    const [screenChanged, setScreenChanged]
    = useState<boolean>(isMobile2)



    useEffect(() => {
       
        if(screenChanged !== isMobile2)
        {
            setResized(true)
            console.warn('resized!!',resized)
        }
         
        
      }, [isMobile2,screenChanged,resized]);
...
const imageVariants: Variants = {
    ...
    useEffect(() => {
        ...
        animate();
    }, [canvasSize, circleRadius,firstCircleComplete,fraction,, circleThickness,resized]);
