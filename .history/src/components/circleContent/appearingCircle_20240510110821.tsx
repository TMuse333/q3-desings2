...
// Modify this effect to conditionally execute based on the resized state
useEffect(() => {
    if (!inView || !redraw || resized) {
    ...
    const intervalId = setInterval(() => {
      if (circleRadius < circleRadiusLimit) {
        // Skip gradual growth animation if resized is true
        const increment = resized ? circleRadiusLimit : 1;
        setCircleRadius(prev => prev + increment);
      } else {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(intervalId);
  }, [fraction, firstCircleComplete, inView, resized]);
