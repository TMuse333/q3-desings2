...
const CircleContent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (secondCircleComplete) {
            ...
            const cp1x = (canvasSize.width / 4 + 00);
            const cp1y = ((canvasSize.height / 5) + 80); // Adjusted y-coordinate
           
        
            // Define the ending point
            const endX = (canvasSize.width / 4 + 270);
            const endY = ((canvasSize.height / 5) + 75);
        
            // Draw the adjusted Bézier curve
            c.beginPath();
            c.moveTo((canvasSize.width / 5) + 90, (canvasSize.height / 5)); // Move to the starting point
            c.bezierCurveTo(cp1x, cp1y, cp1x, cp1y, endX, endY);  // Draw the curve
            c.stroke();
        }
