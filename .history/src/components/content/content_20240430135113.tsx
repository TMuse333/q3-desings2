...
,mainTitle,floatingImage}) => {
    ...
    const imageVariants: Variants = {
        initial: ({ reverse, floatingImage }) => {
            // Define the initial properties of the motion component based on props
            const direction = reverse ? 1 : -1;
            const initialX = direction === -1 ? '-100%' : '100%';
            return {
                x: initialX,
                y: floatingImage ? [10, -10] : 0, // Oscillating movement if floatingImage is true
            };
        },
        animate: {
            x: 0,
...
<h2 className='text-white'>{mainTitle}</h2> */}
            <motion.img
         variants={imageVariants(false,true)}
         initial='initial'
         animate='animate'
