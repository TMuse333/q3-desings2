...
const CircleListElement: React.FC<CircleProps> = ({image,title, description}) => {
    ...
    return (
        ...
        md:flex-row w-screen  md:w-3/4 align-center mb-10 md:mt-3 
   ">
<AppearingCircle
image={image}
/>
...
animate={secondCircleComplete ? 'animate' : 'initial'}
 className="text-white text-xl md:w-[100px]  ml-auto mr-auto mt-5">{title}
 </motion.h1>

<p className={`rounded-lg text-left mt-3 pl-3 pr-3 bg-blue-600 transition-height ease-in duration-300 bg-blue-800 ${descriptionClicked ? ' h-[220px] w-[125px] overflow-scroll pt-5' : 'h-0 w-[125px] border-blue-200 overflow-hidden pt-0'}`}>
  {description}
</p>
</section>
